      ...
      <section className="relative py-24 overflow-hidden" style={{ backgroundColor: "#18181B" }}>
        ...
        <div className="container relative z-10">
          ...
          <h1 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 800, fontSize: "clamp(2.5rem, 6vw, 4.5rem)", color: "#FFFFFF", lineHeight: 1.0, marginBottom: "1.25rem" }}>
            CUSTOM PRESSURE WASHING<br /><span style={{ color: "#F59E0B" }}>RIG BUILDS</span>
          </h1>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={section.title} className="p-5 rounded-lg" style={{ backgroundColor: "#27272A", borderLeft: "4px solid #F59E0B", border: "1px solid rgba(255,255,255,0.06)", borderLeftWidth: "4px", borderLeftColor: "#F59E0B" }}>
                      <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, color: "#F59E0B", fontSize: "1rem", marginBottom: "0.75rem" }}>
                        {section.title}
                      </div>
                      <ul className="space-y-1">
                        {section.items.map((item) => (
                          <li key={item} className="flex items-start gap-2">
                            <CheckCircle size={13} style={{ color: "#F59E0B", marginTop: "3px", flexShrink: 0 }} />
                            <span style={{ color: "#A1A1AA", fontSize: "0.8rem", fontFamily: "'DM Sans', sans-serif" }}>{item}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={type.title} className="p-5 rounded-lg" style={{ backgroundColor: "#27272A", border: "1px solid rgba(255,255,255,0.06)" }}>
                      <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, color: "#FFFFFF", fontSize: "1.05rem", marginBottom: "0.5rem" }}>
                        {type.title}
                      </div>
                      <p style={{ color: "#71717A", fontSize: "0.875rem", lineHeight: 1.6, fontFamily: "'DM Sans', sans-serif" }}>{type.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="p-5 rounded-lg" style={{ backgroundColor: "#27272A", border: "1px solid rgba(255,255,255,0.06)" }}>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <Link key={link.href} href={link.href} className="flex items-center gap-2 text-sm transition-colors hover:text-yellow-400" style={{ color: "#A1A1AA", fontFamily: "'DM Sans', sans-serif" }}>
                      <ArrowRight size={12} style={{ color: "#F59E0B" }} /> {link.label}
                    </Link>
                  ))}
                </div>
              </div>
